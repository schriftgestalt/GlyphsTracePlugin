...
at_bitmap_type input_bmp_reader(at_string filename, at_input_opts_type *opts, at_msg_func msg_func,
								at_address msg_data) {
	FILE *fd;
	...
	if ((Bitmap_Head.biHeight == 0 || Bitmap_Head.biWidth == 0) || (Bitmap_Head.biPlanes != 1) ||
	...
	image_storage = ReadImage(fd, Bitmap_Head.biWidth, Bitmap_Head.biHeight, ColorMap,
							  Bitmap_Head.biBitCnt, Bitmap_Head.biCompr, rowbytes, Grey);
...
static int ReadColorMap(FILE *fd, unsigned char buffer[256][3], int number, int size, int *grey,
...
static short ToS(unsigned char *puffer) { return ((short)(puffer[0] | puffer[1] << 8)); }
