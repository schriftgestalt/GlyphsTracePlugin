...
static void change_bad_lines (spline_list_type *,
  fitting_opts_type *);
static void filter_ (curve_type, fitting_opts_type *);
		...
		at_exception_type * exception)
{
  ...
  for (this_curve = 0; this_curve < curve_list.length; this_curve++)
    {
      LOG1 ("#%u: ", this_curve);
      filter_ (CURVE_LIST_ELT (curve_list, this_curve), fitting_opts);
    }
...
static void filter_ (curve_type curve, fitting_opts_type *fitting_opts) {
